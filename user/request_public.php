...
if(isset($_POST['request_to_donor'])){
    ...
    $stmt->bind_param("iissi", $user_id, $hospital_id, $blood_group, $status, $quantity);
...
.container { margin-top: 60px; max-width: 900px; }
.card-request { background: linear-gradient(145deg,#ffffff,#f8f9fa); border-radius: 20px; padding: 30px; box-shadow: 0 8px 25px rgba(0,0,0,0.15); margin-bottom: 30px; }
.card-request h3 { color: #C41E3A; font-weight: 700; margin-bottom: 25px; text-align: center; }
.form-select { height: 50px; border-radius: 10px; padding-left: 15px; }
.btn-custom { width: 100%; background-color: #C41E3A; color: #fff; border-radius: 10px; font-weight: 600; padding: 12px; margin-top: 20px; transition: 0.3s; }
.btn-custom:hover { background-color: #900020; }
.alert-success { text-align: center; font-weight: 600; border-radius: 12px; padding: 12px; }

.card-donor { border-radius: 15px; padding: 20px; background: #fff; box-shadow: 0 6px 20px rgba(0,0,0,0.1); margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center; }
.card-donor h5 { color: #C41E3A; font-weight: 600; margin-bottom: 5px; }
.card-donor p { margin: 0; }
.donor-info { flex: 1; }
.donor-actions { text-align: right; }
.donor-actions form, .donor-actions a { margin-left: 5px; }
.donor-actions button { margin-bottom: 5px; }
</style>
</head>
<body>
